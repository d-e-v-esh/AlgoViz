...
public class APathfinding {
    ...
    // Elevate the below ArrayList<Node> and type Node variables to a parent class.
    ...
    public APathfinding(GridView gridView, int size) {
    ...
    public APathfinding(GridView gridView, int size, Node start, Node end) {
        this.gridView = gridView;
        this.size = size;
        startNode = start;
        endNode = end;
    ...
    public void calculateNodeValues(int possibleX, int possibleY, Node openNode, Node parent) {
        ...
        if (possibleX < 0 | possibleY < 0 | possibleX >= gridView.getGridWidth() | possibleY >= gridView.getGridHeight()) {
            return;
        }
